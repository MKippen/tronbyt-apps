...
def get_value_color(state, config):
    normal = config.get("normal_color") or "#FFFFFF"
    if not is_numeric(state):
        return normal
    val = float(state)
    above_val = config.get("above_value") or ""
    below_val = config.get("below_value") or ""
    if above_val and is_numeric(above_val) and val > float(above_val):
        return config.get("above_color") or "#FF4444"
    if below_val and is_numeric(below_val) and val < float(below_val):
        return config.get("below_color") or "#4488FF"
    return normal
...
def main(config):
    ...
    label = config.get("label") or friendly_name or entity_id
    display_unit = config.get("unit") or unit or ""
    value_color = get_value_color(state, config)
    display_value = format_value(state, display_unit)

    emoji_code = config.get("emoji") or "1F321-FE0F"
    icon_img = load_emoji(emoji_code)
    bg_color = config.get("icon_bg_color") or "#1C1C2E"

    return render_card(icon_img, display_value, label, value_color, bg_color, False)
